import BasePage from '../basePage';
import loginPageSelectors from './loginPageSelectors';

export class LoginPage extends BasePage {
	constructor() {
		super(); // wymagane przy dziedziczeniu
	}

	login(email = 'administrator@testarena.pl', pass = 'sumXQQ72$L') {
		cy.get(loginPageSelectors.email).type(email);
		cy.get(loginPageSelectors.pass).type(pass);
		cy.get(loginPageSelectors.loginBtn).click();
	}

	checkLoginPageElems() {
		if (cy.url().should('include', 'http://demo.testarena.pl/zaloguj')) {
			cy.get(loginPageSelectors.email).should('be.visible');
			cy.get(loginPageSelectors.pass).should('be.visible');
			cy.get(loginPageSelectors.loginBtn).should('be.visible');
		}
	}
}

export const loginPage = new LoginPage();
